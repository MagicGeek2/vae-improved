...
class VQLPIPSWithDiscriminator(nn.Module):
    ...
    def forward(self,
        ...
        if optimizer_idx == 0:
            ...
            if exists(clip_feat):
                clip_loss = (clip_target.contiguous() -
                             clip_feat.contiguous()).pow(2).mean()
                loss += self.clip_weight * clip_loss
                log[f"{split}/clip_loss"] = clip_loss.detach().mean()
                
            if exists(codebook_loss):
                loss += self.codebook_weight * codebook_loss.mean()
                log[f"{split}/quant_loss"] = codebook_loss.detach().mean()
                
            if exists(predicted_indices):
                assert exists(self.n_classes)
                with torch.no_grad():
                    perplexity, cluster_usage = measure_perplexity(
                        predicted_indices, self.n_classes)
                log[f"{split}/perplexity"] = perplexity
                log[f"{split}/cluster_usage"] = cluster_usage.to(
                    dtype=torch.float32)
